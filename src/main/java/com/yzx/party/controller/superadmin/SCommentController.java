...
public class SCommentController {
    ...
    //展示所有的评论
    @GetMapping("/comments")
    public String comments(@PageableDefault(size = 6,sort = {"creattime"},direction = Sort.Direction.DESC) Pageable pageable,
        ...
        return "superadmin/commentshow";
    }

    //展示所有的评论
    @PostMapping("/comments/search")
    @OperationLogAnnotation(operaType = "搜索",operaDesc = "搜索相关新闻评论")
    public String search(@PageableDefault(size = 6,sort = {"creattime"},direction = Sort.Direction.DESC) Pageable pageable,
                           Model model, QueryCS queryCS, HttpSession session){
        Admins loginAdmin = (Admins)session.getAttribute("admin");
        //默认只能看见学校办公室
        Integer college_id = 1;
        ...
        queryCS.setCollege_id(college_id);
        model.addAttribute("page",commentService.listComments(pageable, queryCS));
        return "superadmin/commentshow :: CSList";
    }


    //删除评论
    @GetMapping("/deleteComment/{id}")
    @OperationLogAnnotation(operaType = "删除",operaDesc = "删除单条新闻评论")
    public String deleteComment(@PathVariable Integer id, RedirectAttributes attributes){
        ...
        return "redirect:/admin/superAdmin/comments";
    }


    //批量删除评论
    @GetMapping("/deleteSomeComment/{checkedID}")
    @OperationLogAnnotation(operaType = "删除",operaDesc = "批量删除新闻评论")
